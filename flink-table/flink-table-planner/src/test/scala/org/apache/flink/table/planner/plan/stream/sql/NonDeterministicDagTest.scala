...
import org.apache.flink.table.planner.utils.{PlanKind, StreamTableTestUtil, TableTestBase}
...
class NonDeterministicDagTest(nonDeterministicUpdateStrategy: NonDeterministicUpdateStrategy)
  ...
  def testCdcCorrelateNonDeterministicFuncSinkWithPK(): Unit = {
    ...
    util.tableConfig.set("table.exec.state.ttl", "3600")
    verifyPlan(s"""
                  |insert into sink_with_pk
                  |select
                  |  t1.a, t1.b, a1
  ...
  private def verifyPlan(query: String, statementSet: Boolean = false): Unit = {
    if (statementSet) {
      util.verifyExplain(query)
    } else {
      util.verifyExecPlanInsert(query)
    }
    util.verifyAnalyzedPlan(query)
  }
